...
class LocalStorageManager {
  ...
  static deleteRepository(repoName) {
    const repos = this.getRepositories();
...
function setupCodeEditors() {
  if (typeof CodeMirror !== 'undefined') {
    const editorConfig = {
      ...
      extraKeys: {
        "Ctrl-S": function(cm) {
          if (!document.getElementById('fileEditor').classList.contains('hidden')) {
            saveFile();
          }
        }
      }
  ...
  if (currentState.repositories.length === 0) {
    repoList.innerHTML = `
      <div class="col-span-full text-center py-12">
        <svg class="w-12 h-12 mx-auto text-github-fg-muted mb-4" fill="currentColor" viewBox="0 0 16 16">
  ...
  currentState.repositories.forEach(repo => {
    ...
    repoCard.innerHTML = `
      <div class="flex items-start justify-between">
        <div class="flex-1">
          <h3 class="text-lg font-semibold text-github-accent-fg mb-1">${repo.name}</h3>
          <p class="text-sm text-github-fg-muted mb-2">${repo.description || 'No description'}</p>
          <div class="flex items-center space-x-4 text-xs text-github-fg-muted">
            <span>${formatDate(repo.created)}</span>
            <span class="flex items-center space-x-1">
              <div class="w-3 h-3 rounded-full bg-github-accent-fg"></div>
              <span>${repo.defaultBranch || 'main'}</span>
            </span>
          </div>
        </div>
        <button onclick="event.stopPropagation();deleteRepository('${repo.name}')" class="text-github-danger-fg hover:text-red-500 p-1">
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="3" class="px-4 py-8 text-center text-github-fg-muted">
          ...
          <button onclick="showCreateFileModal()" class="mt-2 text-github-accent-fg hover:underline text-sm">Create your first file</button>
        </td>
      </tr>
    `;
    return;
    ...
    row.innerHTML = `
      <td class="px-4 py-3">
        <div class="flex items-center space-x-3">
          ${fileIcon}
          <span class="text-github-accent-fg hover:underline font-medium">${file.name}</span>
        </div>
      </td>
      <td class="px-4 py-3 text-github-fg-muted text-sm max-w-md truncate">
        ${file.lastCommit || 'Initial commit'}
      </td>
      <td class="px-4 py-3 text-github-fg-muted text-sm text-right">
...
function getFileIcon(filename, type) {
  if (type === 'folder') {
    return `<svg class="w-4 h-4 text-github-accent-fg" fill="currentColor" viewBox="0 0 16 16">
  ...
  return `<svg class="w-4 h-4" style="color: ${iconColor}" fill="currentColor" viewBox="0 0 16 16">
...
function displayFileContent(filename, fileData) {
  ...
  if (fileData.tags && fileData.tags.length > 0) {
    tagsContainer.innerHTML = fileData.tags.map(tag => `
      <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-github-accent-emphasis/20 border border-github-accent-emphasis/30 text-github-accent-fg">
        ${tag}
      </span>
    `).join('');
  } else {
...
function showContextMenu(x, y, fileName, fileType) {
  ...
  let html = `
    <button onclick="viewFileFromContext('${fileName}')" class="w-full text-left px-4 py-2 text-sm text-github-fg-default hover:bg-github-canvas-subtle flex items-center space-x-2">
      <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 16 16">
...
function setupKeyboardShortcuts() {
  document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Escape') {
      const modals = [
        'createFileModal', 'createRepoModal', 'deleteFileModal'
      ];

...
class Components {
  ...
  showModal(options) {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        const input = modalOverlay.querySelector("input");
  ...
  createFileUpload(options) {
    const { containerId, accept = "*", multiple = false, onUpload } = options;
  ...
  createSearch(containerId, options) {
    const { placeholder = "Search...", onSearch, debounceDelay = 300 } = options;

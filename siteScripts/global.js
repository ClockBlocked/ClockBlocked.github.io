...
const ACTION_GRID_ITEMS = [
  ...
  { id: "add-playlist", icon: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012 2v2M7 7h10", label: "Add to Playlist" },
  {
    id: "share",
    icon:
      "M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3",
    label: "Share",
  },
...
const TOAST_ICONS = {
  [NOTIFICATION_TYPES.SUCCESS]:
    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414L9 13.414l4.707-4.707z" clip-rule="evenodd"/></svg>',
  [NOTIFICATION_TYPES.ERROR]:
    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>',
  [NOTIFICATION_TYPES.WARNING]:
    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.742-2.98L8.257 3.1zM9 8a1 1 0 112 0v4a1 1 0 11-2 0V8zm1 8a1.25 1.25 0 100-2.5A1.25 1.25 0 0010 16z" clip-rule="evenodd"/></svg>',
  [NOTIFICATION_TYPES.INFO]:
    '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M18 10A8 8 0 112 10a8 8 0 0116 0zM9 9a1 1 0 102 0v4a1 1 0 10-2 0V9zm1-3a1 1 0 100 2 1 1 0 000-2z"/></svg>',
};
...
const utils = {
  ...
  getSimilarArtists: (artistName, { limit = 12, includeSelf = false } = {}) => {
    const lib = Array.isArray(typeof music !== "undefined" ? music : null) ? music : Array.isArray(window.MyTunesApp?.music) ? window.MyTunesApp.music : [];
...
const notifications = {
  ...
  show(message, type = NOTIFICATION_TYPES.INFO, undoCallback = null, options = {}) {
    ...
    toast.addEventListener("touchcancel", (e) => {
      touchEnd(e);
      resume();
    });
...
const musicPlayer = {
  mainPlayer: {
    ...
    preventHorizontalScroll: () => {
      ...
      musicPlayerElement.addEventListener(
        "wheel",
        (e) => {
...
const playlists = {
  ...
  remove: async (playlistId) => {
    ...
    const confirmed = await overlays.dialog.confirm(`Delete the playlist "${playlist.name}"? This cannot be undone.`, {
      okText: "Delete",
      danger: true,
    });
  ...
  showAll: () => {
    if (appState.playlists.length === 0) {
      overlays.viewer.playlists(views.renderEmptyState("No Playlists", "You haven't created any playlists yet.", "Create your first playlist to organize your music."));
      return;
    }
                      ...
                      <div class="flex gap-2">
                        ...
                        <button class="delete-playlist-btn px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors" data-playlist-id="${playlist.id}">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
                            <path fill-rule="evenodd" d="M16.5 4.478v.227a48.816 48.816 0 013.878.512.75.75 0 11-.256 1.478l-.209-.035-1.005 13.07a3 3 0 01-2.991 2.77H8.084a3 3 0 01-2.991-2.77L4.002 6.66l-.209.035a.75.75 0 11-.256-1.478c1.16-.186 2.385-.345 3.878-.512V4.478A1.5 1.5 0 018.088 3h7.824a1.5 1.5 0 011.588 1.478zM9 9a.75.75 0 000 1.5h6a.75.75 0 000-1.5H9z" clip-rule="evenodd"/>
                          </svg>
                        </button>
                      </div>
                    </div>
                  ...
                  <div class="playlist-header mb-8 flex items-start gap-6">
                    ...
                    <div class="playlist-info flex-1">
                      ...
                      <div class="flex gap-4">
                        <button class="play-playlist-btn bg-accent-primary text-white px-8 py-3 rounded-full hover:bg-accent-secondary transition-colors flex items-center gap-2" data-playlist-id="${playlist.id}" ${playlist.songs.length === 0 ? "disabled" : ""}>
                          ${ICONS.play}
                          Play
                        </button>
                  ...
                  `
                    ...
                    <div class="songs-list">
                      ...
                      ${playlist.songs
                        .map(
                          (song, index) => `
                        <div class="song-row grid grid-cols-12 gap-4 items-center px-4 py-3 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group" data-song='${JSON.stringify(song)}' data-playlist-id="${playlist.id}" data-song-index="${index}">
                          ...
                          <div class="col-span-1 flex items-center justify-between">
                            ...
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                              ...
                              <button class="action-btn p-1 hover:bg-white/10 rounded transition-colors" data-action="remove-from-playlist" title="Remove from playlist">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v3"/>
                                </svg>
                              </button>
                            </div>
                          </div>

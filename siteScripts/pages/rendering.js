...
import { homePage } from './statics.js';
import { views } from './statics.js';
  ...
  loadHomePage: () => {
    ...
    if (appState.homePageManagerInstance) {
      ...
      setTimeout(() => {
        ...
        // Hide loading system when content is ready
        setTimeout(() => {
          pageLoader.complete();
        }, 700);
    ...
    console.log('Used loadHomePage Function');      
  },
...
loadArtistPage: (artistData) => {
  ...
  setTimeout(() => {
    ...
    pageLoader.complete();  // ← Call IMMEDIATELY after render, not 500ms later
    pageManager.dependencies.scrollToTop();
  }, 200);
},
...
export const pageLoader = {
  ...
  init() {
    ...
    if (!pageLoader.contentContainer) {
      ...
      // Add blur container class if not present
  ...
  start() {
    ...
    // Show center spinner immediately
    ...
    // Start bar animation after a tiny delay (YouTube behavior)
    setTimeout(() => {
  ...
  animateProgressBar() {
    ...
    // YouTube-style progress animation
    const intervals = [
    ...
    intervals.forEach((interval, index) => {
      const timer = setTimeout(() => {
        ...
        // If this is the last interval, start the final slow crawl
  ...
  startFinalCrawl() {
    ...
    // Slow crawl from 0.9 to 0.99 (YouTube behavior)

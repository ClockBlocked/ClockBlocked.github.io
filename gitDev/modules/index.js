(function() {
    ...
    function extractExports(scriptId) {
        const knownExports = {
            ...
            'listeners': ['EventBus', 'on', 'off', 'emit'],
            ...
            'coder': ['CodeEditor', 'SyntaxHighlighter']
        };
    ...
    function renderScriptTable() {
        ...
        SCRIPT_REGISTRY.scripts.forEach((data, id) => {
            const retryButton = data.status === 'Failed'
                ? `%cRetry%c → gitDevLoader.retry('${id}')`
                : '—';
        ...
        if (failedScripts.length > 0) {
            ...
            failedScripts.forEach(([id, data]) => {
                console.log(
                    `%c🔄 Retry ${id}`,
                    'color: #3b82f6; font-weight: bold; cursor: pointer; text-decoration: underline;'
                );
        ...
        for (const manifest of sortedManifest) {
            try {
                ...
                if (manifest.critical) {
                    console.error(`Critical script "${manifest.id}" failed to load!`);
                }
            }
        }

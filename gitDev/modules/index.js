(function() {
  const SCRIPT_REGISTRY = { scripts: new Map(), loadOrder: [], startTime: performance. now() };
  ...
  function generateCacheBuster() { return `v=${Date.now()}`; }
  function formatBytes(bytes) { if (bytes === 0) return "0 B"; const k = 1024; const sizes = ["B", "KB", "MB"]; const i = Math.floor(Math. log(bytes) / Math.log(k)); return parseFloat((bytes / Math.pow(k, i)).toFixed(2)) + " " + sizes[i]; }
  function formatDuration(ms) { if (ms < 1000) return `${Math.round(ms)}ms`; return `${(ms / 1000).toFixed(2)}s`; }
  function extractExports(scriptId) {
    const knownExports = {
      ...
      storage: ["StorageManager", "LocalDB", "SessionCache", "LocalStorageManager"],
      router: ["LoadingProgress", "LoadingSpinner", "showRepoSelector", "showExplorer", "showFileViewer", "showFileEditor", "navigateToRoot", "navigateToPath", "navigateToFolder"],
      pageUpdates: ["updateSelectedTags", "updateBreadcrumb", "updateEditorMode", "updateCommitMessage", "renderRepositoryList", "renderFileList", "displayFileContent", "updateRecentFilesUI", "updateStats"],
      overlays:  ["modal", "dropdown", "showContextMenu", "hideContextMenu", "showCreateRepoModal", "hideCreateRepoModal", "showCreateFileModal", "hideCreateFileModal", "showDeleteFileModal", "hideDeleteFileModal", "showLoading", "hideLoading", "showSuccessMessage", "showErrorMessage"],
      core: ["currentState", "createRepository", "deleteRepository", "createFile", "deleteCurrentFile", "viewFile", "editFile", "saveFile", "openRepository", "initializeApp"],
  ...
  function autoExposeGlobals(scriptId) {
    ...
    const exposed = [];
    expectedExports.forEach(name => { if (typeof window[name] !== "undefined") exposed.push(name); });
    return exposed;
  }
  function loadScript(manifest) {
    return new Promise((resolve, reject) => {
      const scriptData = { id: manifest.id, url: manifest.url, category: manifest.category, priority: manifest.priority, critical: manifest. critical, status: "Pending", startTime: performance.now(), endTime: null, duration: null, size: null, retryCount: 0, maxRetries: 3, exports: [], error: null };
      ...
      script.onload = () => {
        ...
        fetch(manifest.url, { method: "HEAD" }).then(response => { const size = response.headers.get("content-length"); if (size) scriptData.size = parseInt(size, 10); }).catch(() => {});
        resolve(scriptData);
      };
      script.onerror = error => { scriptData.status = "Failed"; scriptData.error = error; scriptData.endTime = performance.now(); scriptData.duration = scriptData.endTime - scriptData.startTime; reject(scriptData); };
  ...
  function retryScript(scriptId) {
    ...
    return loadScript(manifest).then(data => { renderScriptTable(); return data; });
  }
  function renderScriptTable() {
    ...
    console.log("%c╚══════════════════════════════════════════════════════════════════════════╝", "color:  #3b82f6; font-weight: bold;");
    console.log("");
    console.log(`%c📦 Total: ${totalCount} %c│ %c✅ Loaded: ${loadedCount} %c│ %c❌ Failed:  ${failedCount} %c│ %c⏱️ Time: ${formatDuration(totalTime)}`, "color: #6366f1; font-weight:  bold;", "color: #4b5563;", "color: #10b981; font-weight:  bold;", "color:  #4b5563;", "color:  #ef4444; font-weight: bold;", "color: #4b5563;", "color: #f59e0b; font-weight: bold;");
    console.log("");
    const tableData = [];
    SCRIPT_REGISTRY.scripts. forEach((data, id) => { tableData.push({ "#":  data.priority, "Cat. ": data.category, Module: id, Status: data.status, Time: data.duration ?  formatDuration(data.duration) : "—", Size: data.size ? formatBytes(data.size) : "—", Retries: data. retryCount > 0 ? data. retryCount : "—", Critical: data.critical ?  "⚠️" : "—", Exports: data. exports.length > 0 ? data. exports.join(", ") : "—" }); });
  ...
  async function loadAllScripts() {
    ...
    for (const manifest of sortedManifest) {
      try { await loadScript(manifest); }
      catch (error) { if (manifest.critical) console.error(`Critical script "${manifest.id}" failed to load! `); }
    }
    renderScriptTable();
    setTimeout(() => {
  ...
  window.gitDevLoader = {
    ...
    retry: function(scriptId) {
      return retryScript(scriptId).then(() => { console.log(`%c✅ Successfully reloaded "${scriptId}"`, "color: #10b981; font-weight: bold;"); }).catch(err => { console.error(`❌ Failed to reload "${scriptId}": `, err.message); });
    },
    retryAll: function() {
      const failed = Array.from(SCRIPT_REGISTRY.scripts.entries()).filter(([_, data]) => data.status === "Failed").map(([id]) => id);
      if (failed.length === 0) { console.log("%c✅ No failed scripts to retry", "color: #10b981;"); return Promise.resolve(); }
      return Promise.all(failed. map(id => retryScript(id).catch(() => null))).then(() => { renderScriptTable(); });
    },
    reload: function(scriptId) {
      const scriptData = SCRIPT_REGISTRY.scripts. get(scriptId);
      if (!scriptData) { console.error(`Script "${scriptId}" not found`); return Promise.reject(new Error("Script not found")); }
      const existingScript = document. querySelector(`script[data-script-id="${scriptId}"]`);
      if (existingScript) existingScript.remove();
      SCRIPT_REGISTRY. scripts.delete(scriptId);
      const manifest = SCRIPT_MANIFEST.find(m => m.id === scriptId);
      return loadScript(manifest).then(() => { renderScriptTable(); console.log(`%c✅ Force reloaded "${scriptId}"`, "color: #10b981; font-weight:  bold;"); });
    },
    list: function() {
      const allExports = [];
      SCRIPT_REGISTRY.scripts.forEach((data, id) => { if (data.exports.length > 0) { data.exports.forEach(exp => { allExports.push({ Module: id, Export: exp, Type: typeof window[exp], Available: typeof window[exp] !== "undefined" ?  "✅" : "❌" }); }); } });
      console.table(allExports);
      return allExports;
    },
    get manifest() { return SCRIPT_MANIFEST; },
    get registry() { return Object.fromEntries(SCRIPT_REGISTRY.scripts); },
    getScript: function(scriptId) { return SCRIPT_REGISTRY. scripts.get(scriptId); },
    isLoaded: function(scriptId) { const data = SCRIPT_REGISTRY.scripts. get(scriptId); return data && data.status === "Loaded"; },
    ...
    addScript: function(config) {
      const newManifest = { id: config.id, url: config.url, category: config. category || "Custom", priority: config. priority || SCRIPT_MANIFEST. length + 1, critical: config.critical || false };
      SCRIPT_MANIFEST.push(newManifest);
      return loadScript(newManifest).then(() => { renderScriptTable(); });
    }
  };

class coderViewEdit {
  ...
  init() {
    ...
    const filePage =
      document.querySelector('.pages[data-page="file"]');
    if (!filePage) return;
  ...
  <div class="toolbarGroup">
    ...
    <button id="wrapLinesBtn" class="toolbarButton" title="Wrap Lines">
      <svg class="smallIcon" fill="currentColor" viewBox="0 0 16 16">
        <path
          d="M2 4a1 1 0 1 0 0-2 1 1 0 0 0 0 2Zm3.75-1.5a.75.75 0 0 0 0 1.5h8.5a.75.75 0 0 0 0-1.5Zm0 5a.75.75 0 0 0 0 1.5h8.5a.75.75 0 0 0 0-1.5Zm0 5a.75.75 0 0 0 0 1.5h8.5a.75.75 0 0 0 0-1.5ZM3 8a1 1 0 1 1-2 0 1 1 0 0 1 2 0Zm-1 6a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z"
        />
      </svg>
    </button>
    ...
    <button id="foldAllBtn" class="toolbarButton" title="Fold All">
      <svg class="smallIcon" fill="currentColor" viewBox="0 0 16 16">
        <path
          d="M0 2.75C0 1.784.784 1 1.75 1h12.5c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 14.25 15H1.75A1.75 1.75 0 0 1 0 13.25ZM8 10a.75.75 0 0 1-.53-.22l-2.25-2.25a.751.751 0 0 1 1.06-1.06L8 8.19l1.72-1.72a.751.751 0 0 1 1.06 1.06l-2.25 2.25A.75.75 0 0 1 8 10Z"
        />
      </svg>
    </button>
    <button id="unfoldAllBtn" class="toolbarButton" title="Unfold All">
      <svg class="smallIcon" fill="currentColor" viewBox="0 0 16 16">
        <path
          d="M0 2.75C0 1.784.784 1 1.75 1h12.5c.966 0 1.75.784 1.75 1.75v10.5A1.75 1.75 0 0 1 14.25 15H1.75A1.75 1.75 0 0 1 0 13.25ZM8 6a.75.75 0 0 1 .53.22l2.25 2.25a.751.751 0 0 1-1.06 1.06L8 7.81l-1.72 1.72a.751.751 0 0 1-1.06-1.06l2.25-2.25A.75.75 0 0 1 8 6Z"
        />
      </svg>
    </button>
...
`;
  ...
  injectStyles() {
    ...
    s.textContent = `
#coderWrapper{display:flex;flex-direction:column;height:65svh;min-height:300px;max-height:calc(100vh -
200px);background-color:var(--bgCode);border:1px solid var(--borderDefault);overflow:hidden;position:relative;margin:0
0.5rem;border-bottom-left-radius:8px;border-bottom-right-radius:8px;transition:all 0.3s ease}
...
#codeMirrorContainer .CodeMirror{height:100%!important;width:100%!important;flex:1 1
auto;background-color:var(--bgCode)!important;font-family:'JetBrains Mono','Fira
Code',Consolas,monospace;line-height:1.6} #codeMirrorContainer .CodeMirror-scroll{overflow:auto!important}
#codeMirrorContainer .CodeMirror-gutters{background-color:var(--bgCode)!important;border-right:1px solid
var(--borderDefault)} #codeMirrorContainer .CodeMirror-gutter{background-color:var(--bgCode)!important}
#codeMirrorContainer .CodeMirror-linenumber{color:var(--textMuted);padding:0 8px 0 12px;min-width:40px}
#codeMirrorContainer .CodeMirror-foldgutter{width:16px} #codeMirrorContainer
.CodeMirror-foldgutter-open,#codeMirrorContainer
.CodeMirror-foldgutter-folded{cursor:pointer;color:var(--textMuted);font-size:14px;line-height:1.6;text-align:center;transition:color
0.15s ease} #codeMirrorContainer .CodeMirror-foldgutter-open::after{content:"▾";display:inline-block}
#codeMirrorContainer .CodeMirror-foldgutter-folded::after{content:"▸";display:inline-block} #codeMirrorContainer
.CodeMirror-foldgutter-open:hover,#codeMirrorContainer .CodeMirror-foldgutter-folded:hover{color:var(--textPrimary)}
#codeMirrorContainer .CodeMirror-foldmarker{background-color:var(--bgSecondary);color:var(--textSecondary);border:1px
solid var(--borderDefault);border-radius:4px;padding:0 6px;margin:0 4px;font-size:11px;cursor:pointer}
#codeMirrorContainer .CodeMirror-activeline-background{background-color:var(--bgHover)!important} #codeMirrorContainer
.CodeMirror-activeline-gutter{background-color:var(--bgHover)!important} #codeMirrorContainer
.CodeMirror-selected{background-color:var(--bgSelection)!important} #codeMirrorContainer
.CodeMirror-matchingbracket{color:var(--accentPrimary)!important;background-color:var(--bgHover);border-bottom:2px solid
var(--accentPrimary)} #codeMirrorContainer .CodeMirror-cursor{border-left:2px solid var(--accentPrimary)}
.fileFooter{display:flex;justify-content:space-between;align-items:center;padding:6px
12px;background-color:var(--bgSecondary);border:1px solid var(--borderDefault);border-top:none;margin:0
0.5rem;font-size:12px;color:var(--textSecondary)} .footerStats,.footerRight{display:flex;align-items:center;gap:8px}
...
.toolbarButton.active{background-color:var(--bgHover);color:var(--accentPrimary)}`;
  ...
  setupCodeMirror() {
    ...
    if (!this.elements.codeMirrorContainer || this.codeMirror)
      return;
    const fontSize = parseInt(localStorage.getItem("gitcodr_fontsize")) || 12;
    const savedTheme =
      localStorage.getItem("gitcodr_theme");
    const isDark = savedTheme === "dark" || (!savedTheme &&
      document.documentElement.getAttribute("data-theme") === "dark");
    this.codeMirror =
      CodeMirror(this.elements.codeMirrorContainer, {
        value: "",
        mode: "javascript",
  ...
  loadUserPreferences() {
    const wrap =
      localStorage.getItem("gitcodr_wrapLines");
    if (wrap !== null) {
...
><span>Edit</span>`;
  ...
  displayFile(filename, fileData) {
    ...
    const language = typeof getLanguageName === "function" ? getLanguageName(ext) :
      ext.toUpperCase();
...
${this.currentFile}`;
  ...
  saveChanges() {
    ...
    setTimeout(() => {
      try {
        ...
        const filePath = (window.currentState?.path ? window.currentState.path +
          "/" : "") + this.currentFile;
        ...
        setTimeout(() => {
          ...
          this.elements.commitDescriptionInput &&
  ...
  toggleWrapLines() {
    ...
    this.elements.wrapLinesBtn?.classList.toggle("active",
      this.state.wrapLines);
  }
  adjustFontSize(change) {
    const newSize = Math.max(8, Math.min(32, this.state.fontSize +
      change));
  ...
  updateThemeIcon(isDark) {
    if (this.elements.themeIcon) this.elements.themeIcon.innerHTML = isDark ? '<path
    d = "M9.598 1.591a.75.75 0 0 1 .785-.175 7 7 0 1 1-8.967 8.967.75.75 0 0 1 .961-.96 5.5 5.5 0 0 0 7.046-7.046.75.75 0 0 1 .175-.786Z" /
      > ' : ' < path d = "M8 4a4 4 0 1 1 0 8 4 4 0 0 1 0-8Z" / > '; } openSearch() { if (!this.codeMirror) return;
    this.codeMirror.execCommand("findPersistent");
    setTimeout(() => {
      const input =
        document.querySelector(".CodeMirror-search-field");
      if (input) {
        input.focus();
        input.select();
      }
  ...
  toggleFullscreen() {
      ...
      if (this.isFullscreen) {
        ...
        this.elements.fullscreenIcon && (this.elements.fullscreenIcon.innerHTML =
          ...
          > '); } else { this.elements.coderWrapper.classList.remove("fullscreen"); document.body.style.overflow = "";
          this.elements.fullscreenIcon && (this.elements.fullscreenIcon.innerHTML = '<path
            d = "M3.75 2h2.5a.75.75 0 0 1 0 1.5h-2.5a.25.25 0 0 0-.25.25v2.5a.75.75 0 0 1-1.5 0v-2.5C2 2.784 2.784 2 3.75 2Zm6.5 0h2.5C13.216 2 14 2.784 14 3.75v2.5a.75.75 0 0 1-1.5 0v-2.5a.25.25 0 0 0-.25-.25h-2.5a.75.75 0 0 1 0-1.5ZM3.5 9.75a.75.75 0 0 0-1.5 0v2.5c0 .966.784 1.75 1.75 1.75h2.5a.75.75 0 0 1 0-1.5h-2.5a.25.25 0 0 1-.25-.25Zm9 0a.75.75 0 0 1 1.5 0v2.5A1.75 1.75 0 0 1 12.25 14h-2.5a.75.75 0 0 1 0-1.5h2.5a.25.25 0 0 0 .25-.25Z" /
            > '); } setTimeout(() => this.codeMirror?.refresh(), 100); } formatCode() { if (!this.codeMirror || !this.isEditing)
            return;
            ...
            try {
              ...
              if (formatted !== content) {
                ...
                if (typeof showInfoMessage ===
                  "function") showInfoMessage("No formatting changes");
              }
            }
            catch (error) {
              if (typeof showErrorMessage === "function")
                showErrorMessage("Formatting failed");
            }
          }
          setReadOnly(readOnly) {
            if (!this.codeMirror) return;
            this.codeMirror.setOption("readOnly", readOnly);
            this.codeMirror.getWrapperElement().style.cursor = readOnly ? "default" :
              "text";
          }
          getValue() {

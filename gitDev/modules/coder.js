...
.toolbarButton.active{background-color:var(--bgHover);color:var(--accentPrimary)}`;
  ...
  bindEvents() {
    this.elements.editToggleBtn?.addEventListener("click", () =>
      this.isEditing ? this.cancelEdit() : this.enterEditMode()
    );
  ...
  displayFile(filename, fileData) {
    ...
    if (!this.codeMirror) this.setupCodeMirror();
    setTimeout(() => {
      if (this.codeMirror) {
  ...
  enterEditMode() {
    ...
    LoadingSpinner.show();
    this.isEditing = true;
    this.elements.editToggleBtn.innerHTML = `<svg class="icon" fill="currentColor" viewBox="0 0 16 16"><path d="M8 4a4 4 0 1 1 0 8 4 4 0 0 1 0-8Z"/></svg><span>Cancel</span>`;
    ...
    setTimeout(() => LoadingSpinner.hide(), 300);
  }
  ...
  adjustFontSize(change) {
    ...
    if (clampedSize !== this.state.fontSize) this.setCodeMirrorFontSize(clampedSize);
  }
  ...
  toggleTheme() {
    ...
    this.updateThemeIcon(!isDark);
    if (this.codeMirror) this.codeMirror.setOption("theme", isDark ? "default" : "one-dark");
  }
  ...
  foldAll() {
    ...
    for (let i = 0; i < lineCount; i++) this.codeMirror.foldCode(i, null, "fold");
  }

  unfoldAll() {
    if (!this.codeMirror) return;
    const lineCount = this.codeMirror.lineCount();
    for (let i = 0; i < lineCount; i++) this.codeMirror.foldCode(i, null, "unfold");
  }
  ...
  toggleFullscreen() {
    ...
    if (!this.isFullscreen) {
      ...
      this.elements.fullscreenIcon.innerHTML =
        '<path d="M2.75 2A.75.75 0 0 1 3.5 2.75v2.5a.75.75 0 0 1-1.5 0v-2.5A.75.75 0 0 1 2.75 2Zm.75 6.25a.75.75 0 0 0-1.5 0v2.5a.75.75 0 0 0 .75.75h2.5a.75.75 0 0 0 0-1.5H4V8.25Zm6-6.5a.75.75 0 0 0-.75.75v2.5a.75.75 0 0 0 1.5 0v-2.5A.75.75 0 0 0 9.5 2Zm3.25 6.25a.75.75 0 0 1 1.5 0v2.5a.75.75 0 0 1-.75.75h-2.5a.75.75 0 0 1 0-1.5H13V8.25Z"/>';

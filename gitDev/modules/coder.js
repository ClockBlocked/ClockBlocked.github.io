class coderViewEdit {
  ...
  init() {
    ...
    if (window.coderArea && window.coderArea.template) {
      coder.innerHTML = window.coderArea.template;
      
      if (!document.querySelector('style[coder-styles]')) {
        const style = document.createElement('style');
        style.setAttribute('coder-styles', 'true');
        style.textContent = window.coderArea.styles;
        document.head.appendChild(style);
      }
    } else {
      console.warn('coderArea not found, using fallback HTML');
      coder.innerHTML = `...fallback HTML...`;
    }

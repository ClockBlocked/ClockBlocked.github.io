  ...
  const editorConfig = {
    ...
    extraKeys: {
      "Ctrl-S": cm => {
        ...
        if (fileView && ! fileView.classList.contains("hidden")) saveFile();
      },
...
function openRecentFile(repoName, filePath, fileName) {
  ...
  setTimeout(() => {
    try {
      ...
      renderFileList();
      updateBreadcrumb();
      const navRepoName = document.querySelector(". navigation . navButton");
      if (navRepoName) navRepoName.textContent = repoName;
      viewFile(fileName);
      LoadingProgress.hide();
    } catch (error) {
      LoadingProgress.hide();

window.addEventListener("DOMContentLoaded", () => {
    ...
    function retryScript(url) {
        ...
        loadScript(scriptRegistry.find(s => s.url === url)).then(renderConsoleTable);
    }
    ...
    function renderConsoleTable() {
        const tableData = scriptRegistry.map(entry => ({
            ...
            Retry: scriptStatusMap[entry.url] === "Error" ? `retryScript("${entry.url}")` : ""
        }));
        console.groupCollapsed("📦 Script Load Status");
        console.table(tableData);
        console.log("To retry a failed script, run:", "\nretryScript(\"<url>\")");
        console.groupEnd();
    }
